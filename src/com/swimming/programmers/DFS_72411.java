 ...
 */
public class DFS_72411 {

    List<String> array = new ArrayList<>();
    Map<String, Integer> map = new HashMap<>();

    public String[] solution(String[] orders, int[] course) {
        // 단품메뉴 구성 정렬하기(제한사항에 정렬하라고 되어 있음)
    ...
    private void combination(String s, String order, int courseLength) {
        // 탈출
        if (courseLength == 0) {
        ...
        for (int i=0; i<order.length(); i++) {
            combination(s + order.charAt(i), order.substring(i + 1), courseLength - 1);
        }
    }

    public static void main(String[] args) {
        final DFS_72411 solution = new DFS_72411();
        String[] result1 = solution.solution(new String[]{"ABCFG", "AC", "CDE", "ACDE", "BCFG", "ACDEH"}, new int[]{2, 3, 4});
