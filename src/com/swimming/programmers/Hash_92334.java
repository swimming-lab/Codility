...
public class Hash_92334 {
    ...
    public int[] solution2(String[] id_list, String[] report, int k) {
        ...
        // 리포트 정보 만들기
        Map<String, ArrayList<String>> reportInfoMap = new HashMap<>();
        for (String rep : reportSet) {
            String reporter = rep.split(" ")[0];
            String reportee = rep.split(" ")[1];

            ArrayList<String> reportList = reportInfoMap.getOrDefault(reportee, null);
            if (reportList == null) {
                reportList = new ArrayList<>();
            }

            reportList.add(reporter);
            reportInfoMap.put(reportee, reportList);
        }

        // 알림 대상 선정
        Map<String, Integer> notifyMap = new HashMap<>();
        for (ArrayList<String> reportList : reportInfoMap.values()) {
            if (reportList.size() >= k) {
                for (String reporter : reportList) {
                    notifyMap.put(reporter, notifyMap.getOrDefault(reporter, 0) + 1);
                }
            }
        }
        ...
        for (int i=0; i<id_list.length; i++) {
            answer[i] = notifyMap.getOrDefault(id_list[i], 0);
        }
    ...
    public static void main(String[] args) {
        final Hash_92334 solution = new Hash_92334();
        int[] result = solution.solution2(
